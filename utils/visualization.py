...
def plot_per_type_accuracy(
    ...
    ax.axhline(y=overall, color=PALETTE["orange"], linestyle="--", linewidth=1.2,
...
def plot_exploration_steps_histogram(
    ...
    ax.hist(step_counts, bins=bins, color=PALETTE["blue"], edgecolor="white", linewidth=0.8,
            alpha=0.85)
...
def plot_action_distribution(
    ...
    ax.legend(wedges, labels, title="Action Type", loc="center left",
...
def plot_exploration_trajectory(
    ...
    ax.plot(positions[:, 0], positions[:, 1], "o-", color=PALETTE["blue"],
            linewidth=2, markersize=8, markeredgecolor="white")

    # Mark start and end
    ax.scatter(*positions[0],  color="green", s=120, zorder=5,
               edgecolors="white", linewidth=1.5, label="Start")
    ax.scatter(*positions[-1], color="red",   s=120, zorder=5,
