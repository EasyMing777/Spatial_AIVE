...
class Agent:
    ...
    def __init__(
        ...
        qa_model_adapter: Optional[BaseModelAdapter] = None,
        ap_model_name: Optional[str] = None,
        ap_model_adapter: Optional[BaseModelAdapter] = None,
        d_model_name: Optional[str] = None,
        d_model_adapter: Optional[BaseModelAdapter] = None,
        generation_config: Optional[Dict[str, Any]] = None,
        **adapter_kwargs: Any,
    ) -> None:
        self.qa_model_adapter = self._resolve_adapter(
            model_name=qa_model_name, model_adapter=qa_model_adapter,
            role="qa", **adapter_kwargs,
        )
        self.ap_model_adapter = self._resolve_adapter(
            model_name=ap_model_name, model_adapter=ap_model_adapter,
            fallback=self.qa_model_adapter, role="ap", **adapter_kwargs,
        )
        self.d_model_adapter = self._resolve_adapter(
            model_name=d_model_name, model_adapter=d_model_adapter,
            fallback=self.qa_model_adapter, role="d", **adapter_kwargs,
        )
    ...
    def _resolve_adapter(
        ...
        fallback: Optional[BaseModelAdapter] = None,
        role: str = "",
        **adapter_kwargs: Any,
    ) -> BaseModelAdapter:
        ...
        if fallback is not None:
            return fallback
        raise ValueError(
            f"Either model_name or model_adapter must be provided for role '{role}'"
        )

    def _get_adapter(self, model_type: str) -> BaseModelAdapter:
        if model_type == "ap":
            return self.ap_model_adapter
        elif model_type == "qa":
            return self.qa_model_adapter
        elif model_type == "d":
            return self.d_model_adapter
    ...
    def format_prompt(
        ...
        kwargs: Dict[str, Any] = dict(
            ...
            images=images, sys_prompt=sys_prompt,
        )
        if prompt_type == "answer_question":
            ...
            return format_action_planing(
                **kwargs, step_idx=step_idx, exploration_history=exploration_history,
            )
        elif prompt_type == "discriminator":
            return format_discriminator(
                **kwargs, exploration_history=exploration_history, step_idx=step_idx,
            )
        elif prompt_type == "answer_question_exploration":
            return format_answer_question_exploration(
                **kwargs, exploration_history=exploration_history,
            )
        else:
    ...
    def run_ap_model(
        ...
        **generation_kwargs: Any,
        ...
        return self.run_model(
            prompt_type, question=question, answer_choices=answer_choices,
            images=images, model_type="ap", step_idx=step_idx,
            exploration_history=exploration_history, sys_prompt=sys_prompt,
            **generation_kwargs,
        )
    ...
    def run_qa_model(
        ...
        **generation_kwargs: Any,
        ...
        return self.run_model(
            prompt_type, question=question, answer_choices=answer_choices,
            images=images, model_type="qa", exploration_history=exploration_history,
            sys_prompt=sys_prompt, **generation_kwargs,
        )
    ...
    def run_d_model(
        ...
        **generation_kwargs: Any,
        ...
        return self.run_model(
            prompt_type, question=question, answer_choices=answer_choices,
            images=images, model_type="d", exploration_history=exploration_history,
            sys_prompt=sys_prompt, step_idx=step_idx, **generation_kwargs,
        )
