...
class PipelineDefaults:
    ...
    # ---- action discretisation (paper §5.1) ----
    #   translation quantised to 0.25 m x N up to 3 m
    #   rotation quantised to 9 deg x N up to 90 deg
    sampling_interval_meter: float = 0.25
    sampling_interval_angle: int = 9
    max_forward_distance: float = 3.0
    max_turn_angle: int = 90

    # ---- exploration budget (paper sets T = 3) ----
    max_steps_per_question: int = 3
    max_tries_gpt: int = 3
    max_images: int = 2

    # ---- VLM ----
    vlm_qa_model_name: str = "gpt-4o"
    vlm_ap_model_name: str | None = None
    vlm_d_model_name: str | None = None
