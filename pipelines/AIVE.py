...
class SpatialVQAPipelineAIVE(PipelineBase):
    ...
    def _execute_action_sequence(
        ...
        """Execute planned actions through the Dreamer and return the new view.

        The concrete world model is injected via ``self.dreamer`` (constructed
        by :class:`~pipelines.AIVE_baseline.PipelineBase`). Swap ``--dreamer_type``
        to plug in a different backend (``mock`` | ``wan2_2``).

        Args:
            image_path: Current observation :math:`i_t`.
            step_idx: Exploration step index.
            action_sequence: Planned discrete actions :math:`\\tau_t`.
            save_dir: Per-question output directory.

        Returns:
            Path to the imagined next view :math:`\\tilde{i}_{t+1}`, or ``None``
            on failure.
        """
        step_dir = os.path.join(save_dir, f"step_{step_idx + 1}")
        return self.dreamer.generate(
    ...
    def run(self) -> None:
        ...
        for question in self.questions:
            ...
            primary_img_path, helper_img_path = self._load_and_prepare_images(question, save_dir)
        ...
        with open(
            os.path.join(self.model_args.output_dir, "final_log.json"), "w", encoding="utf-8"
        ) as f:
            json.dump(
                {
    ...
    def _image_list(self, primary: str, helper: str | None, num_original: int) -> list[str]:
    ...
    def _handle_direct_answer(
        ...
        qid = question["database_idx"]
        images = self._image_list(primary_img_path, helper_img_path, len(question["img_paths"]))
        ...
        self._dump_llm_interaction(save_dir, None, None, question, response, result, [], None)
        if result == "out of control":
            self.results["parsing_err_stats"]["answer"] += 1
            self.results["parsing_err_stats"]["answer_qid"].append(qid)
            result = "wrong"
    ...
    def _check_if_exploration_needed(
        ...
        for _ in range(self.model_args.max_tries_gpt):
            ...
            if "EXPLORE" in decision:
                ...
                self._dump_exploration_check(
                    save_dir, question, current_image_path, response, False
                )
                return False
            time.sleep(1)
        ...
        self._dump_exploration_check(
            ...
            question,
            current_image_path,
            "VLM decision unclear, defaulting to EXPLORE",
            True,
        )
        return True
        ...
        for attempt in range(self.model_args.max_tries_gpt):
            ...
            if action_sequence is not None:
                ...
                if self._validate_action_sequence(corrected):
                    self._dump_planning_interaction(
                        save_dir, step_idx, question, response, corrected
                    )
                    return corrected
            time.sleep(1)
    ...
    def _parse_action_sequence(self, response: str) -> list[str] | None:
                    ...
                    try:
                        ...
                        if value > 0:
                            if action_type in ("turn-left", "turn-right"):
                                ...
                                max_fwd = float(
                                    getattr(self.model_args, "max_forward_distance", 3.0)
                                )
                                value = max(0.25, min(round(value / 0.25) * 0.25, max_fwd))
    ...
    def _correct_action_values(self, action_sequence: list[str]) -> list[str]:
        ...
        for action in action_sequence:
            ...
            if action_type in ("turn-left", "turn-right"):
                max_turn = int(getattr(self.model_args, "max_turn_angle", 90))
                v: float = round(value / 9) * 9
                v = max(9.0, min(v if v > 0 else 9.0, float(max_turn)))
                corrected.append(f"{action_type} {int(v)}")
            elif action_type == "move-forward":
                max_fwd = float(getattr(self.model_args, "max_forward_distance", 3.0))
                v = round(value / 0.25) * 0.25
                v = max(0.25, min(v if v > 0 else 0.25, max_fwd))
    ...
    def _validate_action_sequence(self, action_sequence: list[str]) -> bool:
        ...
        if total_fwd > self.model_args.max_forward_distance + 0.1:
            return False
        return total_rot <= self.model_args.max_turn_angle + 5
